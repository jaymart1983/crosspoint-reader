...
#if FREEINK_CAP_NETWORK
  // "Manage Fonts" reads like a local feature but is entirely a download screen
  // (the crosspoint-fonts GitHub repo over HTTPS), so it goes with the network.
  // SD fonts dropped in by hand keep working; TextSettings still lists them.
  ...
  if (setting.type == SettingType::TOGGLE && setting.valuePtr != nullptr) {
    ...
    switch (setting.action) {
      ...
      case SettingAction::KeyboardLayouts:
        ...
        break;
#if !FREEINK_CAP_NETWORK
      // The enumerators survive so SettingAction keeps one definition, but no
      // row on this build carries them. Listed explicitly rather than behind a
      // default: so -Wswitch keeps working for everything else.
      case SettingAction::KOReaderSync:
      case SettingAction::OPDSBrowser:
      case SettingAction::Network:
      case SettingAction::CheckForUpdates:
